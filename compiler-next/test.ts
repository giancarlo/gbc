...
import { Token, each, scan } from './scanner.js';
import { Flags } from './parser.js';
import { parseExpression } from './parser-expression.js';
import { BaseNode, ParserApi, formatError, nodeText } from '@cxl/compiler';
...
function ast(
	node: BaseNode<string> & { children?: BaseNode<string>[] },
): string {
...
export default spec('compiler', s => {
	...
	s.test('Parser - Expressions', it => {
		...
		st.setSymbols(
			...
			{ name: 'd', type: 'variable' },
			{ name: 'e', type: 'variable' },
			{ name: 'f', type: 'variable' },
		...
		it.should('parse prefix operators', a => {
			...
			match(a, '~0b100100, ~0xff', '(root (, (~ 0b100100) (~ 0xff)))');
			match(
				a,
				'!false, !true, !!!!false',
	...
	/*s.test('Program', it => {
		it.should('Compile "Hello World"', a => {
			...
			if (sf.errors.length) {
				console.log(...sf.errors.map(e => formatError(e)));
				throw 'Errors found';
			}
			a.equal(
				ast(sf.root),
	...
	s.test('baselines', a => {
		function baseline(
			...
			a.test(testName, a => {
				...
				const sf = program.parser(src);
				if (sf.errors.length) {
					sf.errors.forEach(e => console.log(formatError(e)));
					throw 'Errors found';
				}
