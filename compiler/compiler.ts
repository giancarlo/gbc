...
function compileAssign(node: NodeMap['=']): string {
	const [left, right] = node.children;
	if (left.kind !== ',') return `${text(left)}=${compile(right)}`;
	if (right.kind !== ',')
...
function compilePipe(node: NodeMap['>>']): string {
	...
	for (let i = 0; i < stages.length; i++) {
		...
		if (stage) header += `,${fnIds[i]}=${compile(stage)}`;
	}
	header += ';';
	...
	const body = `for(const _v0 of __iter(${valueId})){${build(0, '_v0')}}`;
	return `(function*(){${header}${body}})()`;
}

function compileDef(node: NodeMap['def']): string {
...
function compileFn(node: NodeMap['fn']): string {
	const parameters = node.parameters && compileEach(node.parameters, ',');
...
export function compile(node: Node): string {
	switch (node.kind) {
		...
		case 'parameter': {
			const name = node.label ? text(node.label) : '$';
...
function dataLabelIndex(
	...
	if (inner.kind === 'propdef')
		return inner.symbol.name === name ? 0 : -1;
	if (inner.kind !== ',') return -1;
	for (let i = 0; i < inner.children.length; i++) {
		const item = inner.children[i];
		if (item?.kind === 'propdef' && item.symbol.name === name)
			return i;
	}
	return -1;
}
